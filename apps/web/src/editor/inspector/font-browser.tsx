...
const FAV_KEY = "opencut.fonts.favourites";
const RECENT_KEY = "opencut.fonts.recent";
  ...
  return (
    <div className={cn("flex min-h-0 flex-col", className)}>
      <div className="flex shrink-0 flex-col gap-2 border-b border-border p-3">
        <div className="relative">
          ...
          <Input
            ...
            placeholder={`Search ${FONT_CATALOGUE.length.toLocaleString()} fonts`}
        ...
        <Input
          ...
          placeholder="Preview sentence (defaults to your text)"
      ...
      <div ref={listRef} className="min-h-0 flex-1 overflow-y-auto [scrollbar-width:thin]" onScroll={onScroll}>
        {showShelves && (
          <>
            <Shelf icon={<Star size={11} />} title="Favourites" names={favourites} preview={preview} current={value} favourites={favourites} onPick={pick} onFav={toggleFavourite} empty="Star a font to keep it here" />
            <Shelf icon={<Clock size={11} />} title="Recently used" names={recents} preview={preview} current={value} favourites={favourites} onPick={pick} onFav={toggleFavourite} empty="Nothing yet" />
            <ShelfTitle>
              All fonts <span className="ml-1 tabular-nums text-muted-foreground/70">{rows.length.toLocaleString()}</span>
            </ShelfTitle>
          </>
        )}
        {!showShelves && (
          <ShelfTitle>
            {rows.length.toLocaleString()} {rows.length === 1 ? "font" : "fonts"}
          </ShelfTitle>
        )}
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
          ...
          <span className="shrink-0">
            {font.styles} {font.styles === 1 ? "style" : "styles"}
          </span>
        </div>
      </div>
      <button
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="xl" className="flex h-[min(660px,82dvh)] flex-col overflow-hidden p-0">
        <DialogHeader className="mb-0 shrink-0 gap-0.5 border-b border-border px-4 py-3">
          <DialogTitle className="text-[13px]">Fonts</DialogTitle>
          <DialogDescription className="text-[11px]">Click a font to apply it to the selected text.</DialogDescription>
        </DialogHeader>
