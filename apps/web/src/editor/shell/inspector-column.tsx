    ...
    >
      <AnimatePresence initial={false} mode="popLayout">
        {open ? (
          ...
          >
            <div className="flex h-9 shrink-0 items-center border-b border-border px-3 text-[12px] font-medium">속성</div>
          ...
          <motion.div
            ...
            style={{ width: RAIL_W }}
          >
            <Tooltip content="속성 (I)" side="left">
              <Button variant="ghost" size="icon-compact" aria-label="속성 패널 열기" onClick={() => actions.setUi({ inspector: true })}>
                <PanelRightOpen />
              </Button>
            </Tooltip>
            <span className="mt-2 text-[10px] tracking-wider text-muted-foreground uppercase [writing-mode:vertical-rl]">속성</span>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.aside>
